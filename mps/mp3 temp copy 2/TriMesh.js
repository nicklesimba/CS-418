 ...
 */
    ...
    */
    loadFromOBJ(fileText, type)
    {
        ...
        // First split file by newlines
        lnArr = fileText.split("\n");
        
        // For every line, parse each element out and put the shits in a buffer
        for (var i = 0; i < lnArr.length; i++) {
            if (lnArr[i][0] == "#") { // If line begins with "#", we just log it and move on.
                console.log(lnArr[i]);
                continue;
            }
            elemArr = lnArr[i].split(/\b\s+(?!$)/);
            ...
            if (elemArr[0] == "f") {
                this.fBuffer.push(parseInt(elemArr[1]-1));
                this.fBuffer.push(parseInt(elemArr[2]-1));
                this.fBuffer.push(parseInt(elemArr[3]-1));
